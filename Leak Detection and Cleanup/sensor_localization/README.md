...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Sensor 1** | `Wire` (Bus 0) | GPIO 23 | GPIO 22 | 400 kHz | `0x68` |
| **Sensor 2** | `Wire1` (Bus 1) | GPIO 6 | GPIO 7 | 400 kHz | `0x68` |

Connect VCC to 3.3V and GND to common ground on the microcontroller.

### Step 2: Firmware Compilation and Upload
1. Open **Arduino IDE**.
2. Set the board to **ESP32C6 Dev Module** under **Tools > Board**.
...
4. Open [`read_sensors/accelerometer/accelerometer.ino`](read_sensors/accelerometer/accelerometer.ino).
5. Connect the ESP32 via USB, select the correct port in **Tools > Port**, and upload the sketch.
6. Open the Serial Monitor at **115200 baud** to verify sensor initialization (`Both sensors are initialized`).
...
├── read_sensors/
│   └── accelerometer/
│       └── accelerometer.ino      # Main ESP32 dual-bus sensor polling sketch (uses DFRobot_BMX160 library)
└── sensor_location/
