...
|---|---|
|&bull; Use the **supported simulated reactor track** to improve control, fault detection, safety logic, state estimation, or visualization.<br>&bull; Build or extend a temperature, dye-concentration, syringe-pump, or water-flow system as part of the **supported physical analogue track** to demonstrate feedback and safety concepts.<br>&bull; Create a **digital reactor control-room simulator** that displays reactor power, coolant conditions, control-rod movement, alarms, and safety state.<br>&bull; Develop an **autonomous control-rod optimization tool** that recommends rod movements while respecting operating limits.<br>&bull; Build a **wireless auxiliary-system sensor network** that monitors pumps, valves, temperature, flow, or environmental conditions.<br>&bull; Design a **redundant sensor validation architecture** that uses voting, sensor fusion, or consistency checks to identify incorrect measurements.<br>&bull; Build a **portable instrumentation training rig** that demonstrates measurement, feedback control, disturbances, alarms, and shutdown behaviour.|&bull; Use the **supported simulated reactor track** to study reactor kinetics, heat transfer, uncertainty, power, temperature, and reactivity.<br>&bull; Use the **supported physical analogue track** to study temperature, flow, mixing, sensor accuracy, and system response.<br>&bull; Develop a **smart sensor health monitoring system** that detects noise, drift, and failures.<br>&bull; Create a **digital twin of an SMR cooling loop** to model coolant flow and heat transfer.<br>&bull; Design a **predictive instrumentation maintenance platform** that anticipates instrument failures.<br>&bull; Develop an **AI-assisted alarm prioritization system** that groups and ranks alarms during abnormal conditions.<br>&bull; Conduct a **human-factors control-panel redesign** that applies cognitive science to reduce operator mistakes.|
